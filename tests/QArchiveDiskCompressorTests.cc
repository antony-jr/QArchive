...
void QArchiveDiskCompressorTests::simpleCompression() {
  ...
  e.addFiles(/*entry name(optional)=*/QFileInfo(Test1OutputFile).fileName(),
             TemporaryFilePath);
...
void QArchiveDiskCompressorTests::compressingMultipleFiles() {
  ...
  e.addFiles(QStringList() << QFileInfo(Test3OutputFile1).fileName()
             ...
             QStringList() << TemporaryFilePath << (TemporaryFilePath + ".v2"));

  QObject::connect(&e, &QArchive::DiskCompressor::error, this,
                   &QArchiveDiskCompressorTests::defaultErrorHandler);
  QSignalSpy finishedSpyInfo(&e, SIGNAL(finished()));
  QSignalSpy progressSpyInfo(
      &e, SIGNAL(progress(QString, int, int, qint64, qint64)));
  e.start();
...
void QArchiveDiskCompressorTests::runningCompressorNonSingleThreaded() {
  QArchive::DiskCompressor e(TestCase5ArchivePath, /*parent=*/nullptr,
...
void QArchiveDiskCompressorTests::compressingTarArchiveWithoutFilters() {
  ...
  e.addFiles(/*entry name(optional)=*/QFileInfo(Test6OutputFile).fileName(),
             TemporaryFilePath);
...
void QArchiveDiskCompressorTests::compressingTarArchiveWithZSTD() {
  ...
  e.addFiles(/*entry name(optional)=*/QFileInfo(Test7OutputFile).fileName(),
             TemporaryFilePath);
...
void QArchiveDiskCompressorTests::compressEmptyFiles() {
  QArchive::DiskCompressor e(TestCase8ArchivePath);

  // Write a Empty File
  QFile TestOutput(TemporaryFilePath);
  QVERIFY((TestOutput.open(QIODevice::WriteOnly)) == true);
  TestOutput.write(QByteArray());
  TestOutput.close();

  e.addFiles(QFileInfo(Test8OutputFile).fileName(), TemporaryFilePath);
  ...
  /* The archive should also exists. */
  QVERIFY(QFileInfo::exists(TestCase8ArchivePath));
}

void QArchiveDiskCompressorTests::compressingSpecialCharacterFiles() {
  QArchive::DiskCompressor e(TestCase9ArchivePath);

  /* Write the file to compress and add it. */
  QFile TestOutput(TemporaryFilePath);
  QVERIFY((TestOutput.open(QIODevice::WriteOnly)) == true);
  TestOutput.write(Test9OutputContents.toUtf8());
  TestOutput.close();

  e.addFiles(/*entry name(optional)=*/QFileInfo(Test9OutputFile).fileName(),
             TemporaryFilePath);
