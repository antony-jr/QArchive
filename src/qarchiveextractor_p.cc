...
#define PASSWORD_NEEDED(a) \
  !qstrcmp(archive_error_string(a), "Passphrase required for this entry")
#define PASSWORD_INCORRECT(a) \
  !qstrcmp(archive_error_string(a), "Incorrect passphrase")
...
class ArchiveFilter {
 public:
  ArchiveFilter() : m_match(archive_match_new(), archive_match_free) {}

  short addIncludePattern(const QString& pattern) {
    if (archive_match_include_pattern_w(
            m_match.data(), pattern.toStdWString().c_str()) != ARCHIVE_OK) {
  ...
  short addExcludePattern(const QString& pattern) {
    if (archive_match_exclude_pattern_w(
            m_match.data(), pattern.toStdWString().c_str()) != ARCHIVE_OK) {
      return ApplyPatternFailed;
    }
    return NoError;
...
QJsonObject getArchiveEntryInformation(archive_entry* entry, bool bExcluded) {
  ...
  qint64 blocks = size / blockSizeInBytes +
                  (size % blockSizeInBytes > 0
                       ? 1
                       : 0);  // add one more block if there is a remainder
...
ExtractorPrivate::ExtractorPrivate(bool memoryMode)
    : b_MemoryMode(memoryMode),
      n_Flags(ARCHIVE_EXTRACT_TIME | ARCHIVE_EXTRACT_PERM |
...
#if ARCHIVE_VERSION_NUMBER >= 3003003
  else if (errorCode == ArchivePasswordIncorrect ||
           errorCode == ArchivePasswordNeeded) {
...
void ExtractorPrivate::start() {
...
#if ARCHIVE_VERSION_NUMBER >= 3003003
      b_Started = false;
      if (errorCode == ArchivePasswordIncorrect ||
          errorCode == ArchivePasswordNeeded) {
  ...
  if (errorCode == NoError) {
    ...
    if (!b_MemoryMode) {
      ...
      emit memoryFinished(
          new MemoryExtractorOutput(std::move(m_ExtractedFiles)));
#ifdef __cpp_lib_make_unique
      m_ExtractedFiles = std::make_unique<QVector<MemoryFile>>();
#else
      m_ExtractedFiles.reset(new QVector<MemoryFile>);
...
short ExtractorPrivate::openArchive() {
  ...
  if (!m_ArchivePath.isEmpty()) {
    ...
    if (!(perm & QFileDevice::ReadUser) && !(perm & QFileDevice::ReadGroup) &&
...
short ExtractorPrivate::extract() {
  ...
  if (!m_ArchiveRead && (b_MemoryMode || !m_ArchiveWrite)) {
    ...
    m_ArchiveRead = QSharedPointer<struct archive>(archive_read_new(),
                                                   ArchiveReadDestructor);
    if (!b_MemoryMode) {
      m_ArchiveWrite = QSharedPointer<struct archive>(archive_write_disk_new(),
    ...
    if (!m_Password.isEmpty()) {
      archive_read_add_passphrase(m_ArchiveRead.data(),
    ...
    if (archiveReadOpenQIODevice(m_ArchiveRead.data(), n_BlockSize,
                                 m_Archive)) {
      m_ArchiveRead.clear();
      m_ArchiveWrite.clear();
    ...
    if (!b_MemoryMode &&
        archive_write_disk_set_options(m_ArchiveWrite.data(), n_Flags)) {
      m_ArchiveRead.clear();
      m_ArchiveWrite.clear();
  ...
  for (;;) {
    ...
    // Report final progress signal after extracting the file fully.
    if (n_BytesTotal > 0 && n_TotalEntries > 0) {
      emit progress(archive_entry_pathname(entry), n_ProcessedEntries,
                    n_TotalEntries, n_BytesProcessed, n_BytesTotal);
    } else {
      emit progress(archive_entry_pathname(entry), 1, 1, 1, 1);
    }
    archive_entry_clear(entry);
    QCoreApplication::
        processEvents();  // call event loop for the signal to take effect.
  }
...
short ExtractorPrivate::writeData(struct archive_entry* entry) {
  ...
  if (!(m_ExtractFilters.empty() ||
        m_ExtractFilters.contains(archive_entry_pathname(entry))) ||
  ...
  if (!b_MemoryMode && !m_OutputDirectory.isEmpty()) {
    ...
    const auto& new_entry = outDir
                                .absoluteFilePath(QString::fromStdWString(
  ...
  if (m_CurrentArchiveEntry != entry) {
    if (!b_MemoryMode) {
      ...
      /// Skip Directories.
      if ((currentNode.getFileInformation()).value("FileType").toString() ==
          "Directory") {
        return NoError;
      }
  ...
  for (;;) {
    ...
    if (!b_MemoryMode) {
      ...
      if ((currentNode.getBuffer())
              ->write(static_cast<const char*>(buff), size) == -1) {
        return ArchiveWriteError;
      }
    }
...
#else
    m_ExtractedFiles->emplace_back(currentNode.getFileInformation(),
...
short ExtractorPrivate::processArchiveInformation() {
  ...
  for (;;) {
    ...
    QString CurrentFile(archive_entry_pathname(entry));
    QJsonObject CurrentEntry = getArchiveEntryInformation(
        entry, m_archiveFilter->isEntryExcluded(entry));
